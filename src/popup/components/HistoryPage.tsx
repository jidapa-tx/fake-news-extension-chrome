...
export function HistoryPage({ onBack, onRecheck }: Props) {
    ...
    <div className="flex flex-col flex-1 bg-white dark:bg-slate-900">
      <div className="flex items-center justify-between px-4 py-3 bg-[#1E40AF]">
        <div className="flex items-center gap-2">
          <button onClick={onBack} aria-label="กลับหน้าหลัก" className="text-white/80 hover:text-white text-sm">
            ← กลับ
          </button>
          <span className="text-white font-semibold text-sm">ประวัติการตรวจสอบ</span>
        </div>
        {history.length > 0 && (
          <button
      ...
      {showConfirm && (
        <div className="mx-4 mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
          <p className="text-xs text-red-700 mb-2">ต้องการลบประวัติทั้งหมดใช่ไหม?</p>
      ...
      <div className="flex flex-col flex-1 overflow-y-auto p-4 gap-2">
        {history.length === 0 ? (
          ...
          history.map(entry => (
            <div
              key={entry.id}
              className="border border-slate-100 dark:border-slate-700 rounded-xl p-3 hover:border-slate-200 dark:hover:border-slate-600 transition-colors dark:bg-slate-800"
            >
              <div className="flex items-start justify-between gap-2">
                <p className="text-xs text-slate-700 dark:text-slate-300 flex-1 line-clamp-2">{entry.query}</p>
                <span
              ...
              <div className="flex items-center justify-between mt-2">
                <span className="text-xs text-slate-400">{timeAgo(entry.checkedAt)}</span>
                <button
                  onClick={() => onRecheck(entry.query)}
